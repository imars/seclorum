...
class ConversationMemory:
    def __init__(self, session_id="default_session", use_json=False):
        ...
        logger.debug(f"SQLite file path: {os.path.abspath(self.db_file)}")
        if self.use_json:
    ...
    def _init_sqlite(self):
        with sqlite3.connect(self.db_file) as conn:
            ...
            """)
            conn.commit()
            logger.debug(f"SQLite table created in {self.db_file}")

    def _sync_from_sqlite(self):
        if not os.path.exists(self.db_file):
            logger.debug(f"No existing SQLite file at {self.db_file} to sync")
            return
        with sqlite3.connect(self.db_file) as conn:
