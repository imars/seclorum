...
function initDrones() {
  ...
  for (let i = 0; i < aiCount; i++) {
    ...
    aiDrone.lastPathUpdate = getTimer() - getSetting('aiDrone.pathUpdateInterval'); // Force initial pathfinding
    scene.add(aiDrone);
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const y = 10; // Fixed Y to match drone height
...
function onKeyDown(event) {
  ...
  switch (event.key) {
    ...
    case 'ArrowDown': case 's':
      ...
      break;
    case 'ArrowLeft':
      playerDrone.controls.left = true;
